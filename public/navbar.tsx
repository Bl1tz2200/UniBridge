...
export function NavBar() { // Returning navbar
  ...
  function openNavMenu(){ // Change state between opened navmenu or closed
        if (burgerState.switch && dropdownMenu.switch){
            ...
            setBurgerState({switch: !burgerState.switch, param:{"topLine": "bg-[#85c05d] rotate-45 translate-y-1", "middleLine": "hidden", "bottomLine": "bg-[#85c05d] rotate-135 -translate-y-1"}})
        } else {
  ...
  return (
    <nav className="sticky mt-0 top-0 w-[100%]">
      <header className="relative flex  p-3 md:p-5 pt-5 justify-between items-center shadow-xl z-10 bg-white">
        <section className="hidden sm:flex text-sm md:text-xl gap-5 justify-start">
          <div><a href="/search" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">Search</a></div>
          <div><a href="/guidelines" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">Guidelines</a></div>
          <div><a href="/community" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">Community</a></div>
        </section>
        <section className="w-3/12 text-2xl md:text-3xl mt-1 mb-1 text-center ">
          <div><a href="/" className="font-(family-name:--font-rubik) hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">UniBridge</a></div>
        </section>
        <section className="hidden sm:flex w-3/12 text-sm md:text-xl gap-5 justify-end">
          <div><a href="/mentor" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">AI-Mentor</a></div>
      ...
      <ul className={`${dropdownMenu.param.moveToView} -mt-35 sm:hidden z-0 grid columns-1 w-screen border-b-1 shadow-xl shadow-black/20 transition-transform duration-500 bg-white`}>
            ...
            <li className="text-center text-l p-0.5"><a href="/mentor" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">AI-Mentor</a></li>
            <li className="text-center text-l p-0.5"><a href="/profile" className="hover:text-[#85c05d] focus:text-[#85c05d] transition-colors duration-200">Profile</a></li>
      </ul>
    </nav>
  );
}